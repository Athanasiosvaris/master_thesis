#!/usr/bin/env python3
"""Parse a1 scaling-experiment architecture2 per-device pulsar logs into CSVs.

logs/ holds one consumer log per device: pulsarConsumer60Batches_device<N>.log
...
DEVICE_RE = re.compile(r"pulsarConsumer60Batches_(device\d+)\.log$")
...
def main():
    logs = sorted(
        glob.glob(os.path.join(LOGS_DIR, "pulsarConsumer60Batches_device*.log")),

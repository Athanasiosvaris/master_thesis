...
def upload_model_to_rustfs(
    ...
    s3_client.upload_file(
        f"./{model_name}",
        bucket_name,
        f"{device_name}/{first_timestamp}-{last_timestamp}-model/{model_name}",
    )
    ...
    s3_client.upload_file(
        f"./scaler.save",
        bucket_name,
        f"{device_name}/{first_timestamp}-{last_timestamp}-model/scaler.save",
    )
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Name of the device for which to train the model.",
    )
    parser.add_argument(
        "--bucket_name",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Name of the database table containing the actual values.",
    )
...
if __name__ == "__main__":
    ...
    print(
        f"Retraining model for device {device_name} with data from table {database_table} between timestamps {first_timestamp} and {last_timestamp}..."
    )

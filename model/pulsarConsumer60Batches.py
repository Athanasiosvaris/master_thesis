...
def insert_data_to_db(conn, batch: pd.DataFrame, table_name: str):
    cur = conn.cursor()
...
def build_batch(rows: list) -> pd.DataFrame:
    """Sort by sensor_timestamp, convert epoch -> datetime, and compute
    message_delay = now (ms) - message_creation_time (ms)."""
    ...
    df["message_delay"] = epoch_now_ms - df["message_creation_time"]
    return df
...
def main():
    parser = argparse.ArgumentParser(
        description="Architecture-2 (no Flink) coordinator: batches of 60 messages"
    )
    parser.add_argument(
        "--topic",
        type=str,
        required=True,
        help="Pulsar topic delivering Sensor records (Avro)",
    )
    parser.add_argument(
        "--device_name",
        type=str,
        required=True,
        help="Device name; used to derive the actualvalues table name",
    )

...
def fetch_actual_values_from_db(conn,database_table_name:str):
    cur = conn.cursor()
    query = f"select sensor_id,sensor_energy_value,sensor_timestamp from {database_table_name} order by sensor_timestamp desc limit 60;"
    cur.execute(query)  
    rows  = cur.fetchall()
...
def forecasting(model,scaler, conn,bucket_name: str,database_table_name: str):
        actual_values_df = dataset_preparation(fetch_actual_values_from_db(conn, "actualvalues"))
...
if __name__ == "__main__":
        ...
        forecasting(model, scaler, conn, args.bucket_name,database_table_name)
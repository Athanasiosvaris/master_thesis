...
def insert_data_to_db(
    con, forecasted_values_df: pd.DataFrame, database_table_name: str
):
    try:
        cur = con.cursor()
...
def fetch_model_and_scaler_from_s3(
    s3_client,
    device_name: str,
    bucket_name: str,
    model_path: str,
    ...
    if not retrain:
        ...
        name = (
            device_name + "/initial/" + device_name + ".keras"
        )  # =>deviceX/initial/deviceX.keras
        scaler_name = device_name + "/" + "initial" + "/scaler.save"
        print(f"Forecasting for initial model using {name} and {scaler_name}")
    else:
        name = device_name + "/" + model_path + "/" + device_name + ".keras"
        scaler_name = device_name + "/" + model_path + "/scaler.save"
        print(
            f"Forecasting for retrained model using {name} and {scaler_name}"
        )

    while True:
        try:
            s3_client.download_file(
                bucket_name, name, f"./{device_name}.keras"
            )
...
def forecasting(
    ...
    database_table_name_actual_values: str,
):
    actual_values_df = dataset_preparation(
        fetch_actual_values_from_db(conn, database_table_name_actual_values)
    )
    ...
    # Send into db
    if bucket_name == "missingtimestamp":
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Forecasting script for sensor data"
    )
    parser.add_argument(
        "--bucket_name",
        type=str,
        ...
        help="Name of the S3 bucket to fetch the model and scaler from",
    )
    parser.add_argument(
        "--device_name",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="Name of the model to fetch from S3 (without file extension)",
    )
    parser.add_argument(
        "--retrain",
        action="store_true",
        help="Whether am forecasting for retrained model or for initial model",
    )
    parser.add_argument(
        "--delete_forecasting_database",
        action="store_true",
        help="Whether am forecasting for retrained model or for initial model",
    )
    ...
    if args.delete_forecasting_database:
    ...
    forecasting(
        ...
        args.bucket_name,
        database_table_name,
        database_table_name_actual_values,
    )
